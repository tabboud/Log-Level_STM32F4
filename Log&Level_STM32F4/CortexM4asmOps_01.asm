...
switch_init:
	...
	SET_bit RCC_AHB1ENR,2					@;      ""         GPIOC
	@;***Switch initializing****		
	PORTBIT_init PULLUP_INPIN,GPIOA_BASE,15	@;	PA15 SW_1-3-5-7-9-11-13
	PORTBIT_init PULLUP_INPIN,GPIOC_BASE,8	@;	PC8	 SW_2-4-6-8-10-12
...
write9: CATHODE_write 0,0,0,0,1,0,0,1
	bx lr
	.thumb_func
writeNEGATIVE: CATHODE_write 1,1,1,1,1,1,0,1			@;Displays the negative sign
	bx lr
	.thumb_func
writeBLANK: CATHODE_write 1,1,1,1,1,1,1,1				@;Displays Nothing! (blank screen)
	bx lr
	.thumb_func
...
@;/------SWITCH dispatch------/
	.global getSWITCH		@;int getSWITCH(int num) //return sampled value of switch
	.thumb_func
getSWITCH:
	@;dispatch via table
	and r0,#0x0F							@;restrict argument to first 16 table entries
	lsl r0,2								@; and convert to table offset
	adr r2,getSWITCH_dispatch_table			@;	get table origin	
	ldr PC,[r2,r0]							@;	  and dispatch selected function from table
	@;the dispatch table
	.align 2	
getSWITCH_dispatch_table: @;destination address must be a .thumb_func
	.word read_dummysw 	@; "" (note: unused switch index -- returns '1' always)
	.word read_sw1 		@; ""
	.word read_sw2 		@; ""
	.word read_sw3 		@; ""
	.word read_sw4 		@; ""
	...
	.word read_sw11 	@; ""
	.word read_sw12 	@; ""
	.word read_sw13 	@; "" (note: unused switch index -- returns '1' always)
	.word read_dummysw 	@; "" 	""
	.word read_dummysw 	@; ""	""
	@;functions for the dispatch table
	.thumb_func
read_dummysw: @;occupies unused entries of the dispatch table
	mov r0,#1	@;dummy switch 0; returns 1 == 'not pressed'
	bx lr
	.thumb_func        				 
read_sw1: SWITCH_read GPIOB_BASE,5, GPIOA_BASE,15		@;At location 0x08000A5E
	bx lr                             
	.thumb_func                       
read_sw2: SWITCH_read GPIOB_BASE,5, GPIOC_BASE,8 
	bx lr                             
	.thumb_func                       
read_sw3: SWITCH_read GPIOB_BASE,11, GPIOA_BASE,15
	bx lr                             
	.thumb_func                       
read_sw4: SWITCH_read GPIOB_BASE,11, GPIOC_BASE,8  
	bx lr                             
	.thumb_func                       
read_sw5: SWITCH_read GPIOB_BASE,0, GPIOA_BASE,15
	bx lr                             
	.thumb_func                       
read_sw6: SWITCH_read GPIOB_BASE,0, GPIOC_BASE,8  
	bx lr                             
	.thumb_func                       
read_sw7: SWITCH_read GPIOB_BASE,1, GPIOA_BASE,15 
	bx lr                             
	.thumb_func                       
read_sw8: SWITCH_read GPIOB_BASE,1, GPIOC_BASE,8  
	bx lr                             
	.thumb_func                       
read_sw9: SWITCH_read GPIOC_BASE,4, GPIOA_BASE,15 
	bx lr                             
	.thumb_func                       
read_sw10: SWITCH_read GPIOC_BASE,4,  GPIOC_BASE,8 
	bx lr                             
	.thumb_func                       
read_sw11: SWITCH_read GPIOC_BASE,5, GPIOA_BASE,15
	bx lr                             
	.thumb_func                       
read_sw12: SWITCH_read GPIOC_BASE,5, GPIOC_BASE,8
	bx lr
	.thumb_func
read_sw13: SWITCH_read GPIOA_BASE,1, GPIOA_BASE,15
	bx lr
...
@;My version of the intended void get_sw() function	
	.global load_SWque		@;int load_SWque(int msTicks, int sw_num, int action);	action: pressed '1' or unpressed '0'
	.thumb_func				@;Returns the value/"event" to hold in que
load_SWque:					@;R0: msTicks;  R1: sw_num;  R2: action
	lsl r0, #8				@;move msTicks to top 3 bytes
	lsl r2, #7				@;move action to que[7]
	orr r0, r0, r2			@;Append msticks and action into on word
	orr r0, r0, r1			@;Append sw_num to msticks and action:(0xTTTTTTAS , Time, Action, Switch)	
bx LR
...
@;/****Rotary encoder****/
	...
	.thumb_func
RE_reset:
	.extern precision_cnt
	ldr r1, =precision_cnt
	mov r2, #0			@;set precision_cnt back to 0
	str r2, [r1]
bx LR
...
@;where displaybuf format: { {cathodes},{anodes} } (separate subarrays of cathodes, anodes)
	.global display_this		@;Must say CATHODE_write 1,1,1,1,1,1,1,1 before calling display this
...
bx LR
	...
	.thumb_func
write_PD:
	PORTBIT_write GPIOB_BASE, 0, 0	@;PB0 - CA_DP	-Write a '0' to decimal point address
	PORTBIT_write GPIOD_BASE, 2, 0	@;PD2 - CA_CLK	-Pulse the clock
	PORTBIT_write GPIOD_BASE, 2, 1	@;PD2 - CA_CLK
	bx LR

@;/****Accelerometer LED Functions****/
	.global level
	.thumb_func
level:
	ANODE_WRITE 1,1,0,1,1,1,1,1		@;AN_G
	CATHODE_WRITE 0,0,0,1,1,0,0,0	@; CA_C, F, G, A, B, DP
bx LR	
	.global turn_left
	.thumb_func
turn_left:
	ANODE_WRITE 1,1,0,1,1,1,1,1		@;AN_G
	CATHODE_WRITE 0,0,1,1,1,0,0,1	@; CA_F, G, A, B
bx LR
	.global turn_right
	.thumb_func
turn_right:
	ANODE_WRITE 1,0,1,1,1,1,1,1		@;AN_R
	CATHODE_WRITE 0,1,0,1,1,1,0,0	@; CA_C, G, A, DP
bx LR
	


@;/****Voltage LED Functions****/
	.global LED_Logging_ON		@;Turns on LED4 Green when logging voltage
	.thumb_func
LED_Logging_ON:
	ANODE_WRITE 1,1,0,1,1,1,1,1		@;AN_G
	CATHODE_WRITE 1,1,1,1,1,0,1,1	@; CA_F
bx LR		

	.global LED_Cutoff_ON		@;Turns on LED2 and 5 Red when Over/Under voltage is active
	.thumb_func
LED_Cutoff_ON:
	ANODE_WRITE 1,0,1,1,1,1,1,1		@;AN_R
	CATHODE_WRITE 0,1,1,1,1,1,0,1	@; CA_A, G
bx LR	

	.global LED_OFF		@;Turns on LED4 Green when logging voltage
	.thumb_func
LED_OFF:
	ANODE_WRITE 1,1,1,1,1,1,1,1		@;AN_G
	CATHODE_WRITE 1,1,1,1,1,1,1,1	@; CA_F
bx LR	
